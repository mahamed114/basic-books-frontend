...
export default function vendors({ vendorData }) {
  return (
    <>
      <Head>
        <title>Vendors | Basic Books</title>
        <meta
          name="description"
              ...
              return (
                ...
                >
                  <>
                    ...
                    <h3 className="">{vendor.ven_company}</h3>
                    <h3 className="">{vendor.ven_email}</h3>
                    <h3 className="">{vendor.ven_mobile}</h3>
                  </>
                </div>
              );
            })}
...
export async function getServerSideProps({ req }) {
  ...
  const vendorData = await fetch(process.env.VENDORS_API, {
    method: "GET",
    headers: {
